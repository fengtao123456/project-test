$(function () {
    //变量的抽取:
     var passwordDatagrid=$('#passwordDatagrid');
     var passwordDlg=$('#passwordDlg');
     var  passwordForm=$('#passwordForm');


    //datagrid
    passwordDatagrid.datagrid({
        url: 'password/list',
        fit:true,
        title:'修改密码',
        rownumbers:true,
        ...
        columns: [[
           ...
           {field: 'name', title: '用户名称', width: '10%',align:  'center'},
    ...
    var cmdObj={
        'add':function () {
            //弹出一个dialog,里面装form表单  passwordDlg
            passwordDlg.dialog('open').dialog('center').dialog('setTitle','部门添加');
            //清空表单
            passwordForm.form('clear');
        },
        'edit':function () {
            //先获取编辑的数据
            var row = passwordDatagrid.datagrid('getSelected');
            //做一个判断:是否选中
            if (row){
                //弹出dialog,进行数据的回显
                //弹出一个dialog,里面装form表单  passwordDlg
                passwordDlg.dialog('open').dialog('center').dialog('setTitle','部门修改');
                //清空表单
                passwordForm.form('clear');
                //表单数据的回显:
                passwordForm.form('load',row);
            }else{
            ...
            if (row){
                $.messager.confirm('温馨提示','你确定删除:[<font color="pink">'+row.name+"</font>]吗?",function(r){
                    ...
                    if (r){
                        ...
                        $.get('password/delete?id='+row.id,function (d) {
                            //d的处理
                          if(d.success){
                              //true:成功
        ...
        'refresh':function () {
            ...
            passwordDatagrid.datagrid('reload');
        },
        'submit':function(){
            //提交:表单的提交:
            passwordForm.form('submit', {
                    url:'password/saveOrUpdate',
                onSubmit: function(){
            },
