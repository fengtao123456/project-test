$(function () {
    //变量的抽取:
     var destorylossDatagrid=$('#destorylossDatagrid');
     var destorylossDlg=$('#destorylossDlg');
     var  destorylossForm=$('#destorylossForm');


    //datagrid
    destorylossDatagrid.datagrid({
        url: 'destoryloss/list',
        fit:true,
        title:'部门管理',
        rownumbers:true,
        pagination:true,
        ...
        columns: [[
           ...
           {field: 'destory', title: '类型', width: '10%',align:  'center',formatter:function (data) {
                   return data.name;
               }},
           {field: 'employee', title: '用户', width: '10%',align:  'center',formatter:function (data) {
                   return data.username;
               }},
           {field: 'appenddate', title: '事故日期', width: '10%',align:  'center'},
    ...
    var cmdObj={
        'add':function () {
            //弹出一个dialog,里面装form表单  destorylossDlg
            destorylossDlg.dialog('open').dialog('center').dialog('setTitle','部门添加');
            //清空表单
            destorylossForm.form('clear');
        },
        'edit':function () {
            //先获取编辑的数据
            var row = destorylossDatagrid.datagrid('getSelected');
            //做一个判断:是否选中
            if (row){
                //弹出dialog,进行数据的回显
                //清空表单
                ...
                destorylossForm.form('load',row);
                //弹出一个dialog,里面装form表单  destorylossDlg
                destorylossDlg.dialog('open').dialog('center').dialog('setTitle','部门修改');
            }else{
            ...
            if (row){
                $.messager.confirm('温馨提示','你确定删除:[<font color="pink">'+row.archive.title+"</font>]吗?",function(r){
                    ...
                    if (r){
                        ...
                        $.get('destoryloss/delete?id='+row.id,function (d) {
                            //d的处理
                          if(d.success){
                              //true:成功
        ...
        'refresh':function () {
            ...
            destorylossDatagrid.datagrid('reload');
        },
        'submit':function(){
            //提交:表单的提交:
            destorylossForm.form('submit', {
                url:'destoryloss/saveOrUpdate',
                onSubmit: function(){
            },
