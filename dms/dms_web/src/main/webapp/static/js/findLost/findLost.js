$(function () {
    //变量的抽取:
     var findLostDatagrid=$('#findLostDatagrid');
     var findLostDlg=$('#findLostDlg');
     var  findLostForm=$('#findLostForm');


    //datagrid
    findLostDatagrid.datagrid({
        url: 'findLost/list',
        fit:true,
        title:'部门管理',
        rownumbers:true,
        ...
        columns: [[
           ...
           {field: 'finddate', title: '找回日期', width: '10%',align:  'center'},
    ...
    var cmdObj={
        'add':function () {
            //弹出一个dialog,里面装form表单  findLostDlg
            findLostDlg.dialog('open').dialog('center').dialog('setTitle','部门添加');
            //清空表单
            findLostForm.form('clear');
        },
        'edit':function () {
            //先获取编辑的数据
            var row = findLostDatagrid.datagrid('getSelected');
            //做一个判断:是否选中
            if (row){
                //弹出dialog,进行数据的回显
                //清空表单
                ...
                findLostForm.form('load',row);
                //弹出一个dialog,里面装form表单  findLostDlg
                findLostDlg.dialog('open').dialog('center').dialog('setTitle','部门修改');
            }else{
            ...
            if (row){
                $.messager.confirm('温馨提示','你确定删除:[<font color="pink">'+row.archive.title+"</font>]吗?",function(r){
                    ...
                    if (r){
                        ...
                        $.get('findLost/delete?id='+row.id,function (d) {
                            //d的处理
                          if(d.success){
                              //true:成功
        ...
        'refresh':function () {
            ...
            findLostDatagrid.datagrid('reload');
        },
        'submit':function(){
            //提交:表单的提交:
            findLostForm.form('submit', {
                    url:'findLost/saveOrUpdate',
                onSubmit: function(){
            },
